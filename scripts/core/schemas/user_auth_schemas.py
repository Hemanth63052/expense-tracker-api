...
class SignUpSchema(BaseModel):
    ...
    last_name: Optional[str] = Field(
        default="", description="This field gets the last name of the user"
    )
    password: str = Field(
        description="This field gets the password of the user and "
        "it is necessary while login"
    )
    email: str = Field(
        description="This field is required to get the email of the "
    ...
    def email_validator(cls, values):
        if (
            re.match(r"^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+$", values["email"].lower())
            is None
        ):
            raise HTTPException(
