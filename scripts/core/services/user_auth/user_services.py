...
def signup(sign_up_payload:SignUpSchema, db_session: Session = Depends(get_db_session)):
    try:
        return AuthenticationHandler().signup(request_payload=sign_up_payload, db_session=db_session)
    except Exception as e:
        logging.info(f"Failed in sign up as {e}")
        return DefaultFailedResponse(message=f"{e}")

@user_router.post("/login")
def login(login_payload:LoginSchema, request:Request, response: Response,
          db_session: Session =Depends(get_db_session)):
    try:
        return AuthenticationHandler().login(login_payload=login_payload,
                                             request=request, response=response,
                                             db_session=db_session)

    except Exception as e:
        logging.info(f"Failed in Login")
        return DefaultFailedResponse(message=f"{e}")
