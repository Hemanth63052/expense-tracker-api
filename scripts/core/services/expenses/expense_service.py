...
def create_category(create_category_payload: CreateExpense,
                    db_session: Annotated[Session, Depends(get_db_session)],
                    meta: MetaInfoSchema = Depends(get_cookies)):
    try:
        return ExpenseHandler(meta.user_id).create_new_expense(db_session=db_session,
                                                               create_payload=create_category_payload)
    except Exception as e:
        logging.exception(f"failed in getting categories for user as {e}")


@expense_router.put("/categories/{cid}/transactions/{tid}")
def update_category(update_expense_payload: CreateExpense, db_session: Annotated[
...
def delete_category(db_session: Annotated[Session, Depends(get_db_session)], cid: str = None, meta: MetaInfoSchema = Depends(get_cookies), tid: str = None):
    try:
