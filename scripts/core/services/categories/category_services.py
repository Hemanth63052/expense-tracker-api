...
from scripts.utils.cookie_decorator import MetaInfoSchema,CookieDecorator
...
def get_categories_list( db_session:Annotated[Session, Depends(get_db_session)],
                         meta: MetaInfoSchema = Depends(get_cookies),):
    try:
        return CategoryHandler(user_id=meta.user_id).get_category_list_for_user(db_session =db_session)
    except Exception as e:
        logging.exception(f"failed in getting categories for user as {e}")


@category_router.get("/categories/{id}")
def get_category_info_by_id(db_session:Annotated[Session, Depends(get_db_session)],
                            meta: MetaInfoSchema = Depends(get_cookies),
                            id: str= None):
    try:
        return CategoryHandler(user_id=meta.user_id).get_category_by_id(db_session=db_session,category_id=id)
    except Exception as e:
        logging.exception(f"failed in getting categories for user as {e}")

@category_router.post("/categories/create")
def create_category(create_category_payload:CategoryCreate,db_session:Annotated[
    Session, Depends(get_db_session)], meta: MetaInfoSchema =Depends(get_cookies)):
    try:
...
def update_category(update_category_payload:CategoryCreate,db_session:Annotated[
    Session, Depends(get_db_session)], meta: MetaInfoSchema =Depends(get_cookies),
                    id:str=None):
    try:
...
def delete_category(db_session:Annotated[Session, Depends(get_db_session)], id:str = None, meta: MetaInfoSchema =Depends(get_cookies)):
    try:
