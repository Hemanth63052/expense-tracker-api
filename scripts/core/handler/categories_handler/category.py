from scripts.core.handler.categories_handler.category_queries import CategoryQueries
...
class CategoryHandler:
    ...
    def get_category_by_id(self, db_session: Session, category_id: str):
        sql_util = SQLUtil(session=db_session)
        data = sql_util.fetch_as_json(self.category_queries.get_category_query(
    ...
    def create_new_category(self, db_session: Session, create_payload: CategoryCreate):
        ...
        sql_util = SQLUtil(session=db_session)
        data = sql_util.fetch_as_json(
            self.category_queries.get_category_query(user_id=self.user_id, title=create_payload.title))
        if data:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Category is already exists")
        sql_util.insert_into_postgres_using_session(create_payload.model_dump(), CategoriesOrmSchema)
        return DefaultResponse(message="Category creation success")

    def update_existing_category(self, db_session: Session, update_payload: CategoryCreate):
        ...
        query_dict = {"user_id":self.user_id, "category_id":update_payload.category_id}
    ...
    def delete_existing_category(self, db_session:Session, category_id:str):
        sql_util = SQLUtil(session=db_session)
        sql_util.delete_data({"user_id":self.user_id,
                                            "category_id":category_id}, table=CategoriesOrmSchema)
